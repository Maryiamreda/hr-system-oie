...
public class EmployeeLeaveRecordsIntegrationTest {
    ...
    private static final LocalDate EMPLOYEE_LEAVE_REQUEST_START_DATE_FUTURE =  LocalDate.of(2026, 12, 1);

    private static final LocalDate EMPLOYEE_LEAVE_REQUEST_START_DATE_FRIDAY = LocalDate.of(2026, 3, 20);
    private static final BigDecimal EMPLOYEE_EXTRA_LEAVE_DAYS_DEDUCTED_MONEY = new BigDecimal("166.67");
    ...
    @Test
    @DatabaseSetup(value = "/dataset/addNewLeaveForEmployee.xml")
    void addNewLeaveForEmployee_WithFutureStartDay_ReturnsBadRequest() throws Exception {
        Employee employee = employeeRepository.findByNationalId(EMPLOYEE_REQUEST_LEAVE_NATIONAL_ID).get();
        LeaveRequestDto leaveRequestDto = LeaveRequestDto.builder().
                days(EMPLOYEE_LEAVE_REQUEST_DAYS)
                .startDate(EMPLOYEE_LEAVE_REQUEST_START_DATE_FUTURE)
                .note(UNIQUE_LEAVE_NOTE)
                .build();
        mockMvc.perform(post(EMPLOYEE_API + "/" + employee.getId() + "/leave")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(leaveRequestDto)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$",containsInAnyOrder(ERROR_NONVALID_LEAVE_START_DATE)));
        //make sure the record with this data and with the expected end date and total days also
        assertThat(leaveRepository.findByNote(UNIQUE_LEAVE_NOTE)).isEmpty();
    }
    ...
    @Test
    @DatabaseSetup(value = "/dataset/addNewLeaveForEmployee_AlreadyExhaustedLeave.xml")
    void addNewLeaveForEmployee_WithExceedsAllowedLeaveDays_ReturnCreatedStatusAndDeductFromSalary() throws Exception {
        Employee employee = employeeRepository.findByNationalId(EMPLOYEE_REQUEST_LEAVE_NATIONAL_ID).get();
        LeaveRequestDto leaveRequestDto = LeaveRequestDto.builder().
                days(EMPLOYEE_LEAVE_REQUEST_DAYS)
                .startDate(EMPLOYEE_LEAVE_REQUEST_START_DATE)
        ...
        List<SalaryRecord> deductionsList = salaryRecordRepository
                .findByEmployeeIdAndTypeAndSalaryMonthAndSalaryYear(
                        employee.getId(),
                        String.valueOf(SalaryRecordType.DEDUCTION),
                        EMPLOYEE_LEAVE_REQUEST_START_DATE.getMonthValue(),
                        EMPLOYEE_LEAVE_REQUEST_START_DATE.getYear()
                );
        assertThat(deductionsList).isNotNull();
        assertThat(deductionsList.get(0).getType()).isEqualTo(String.valueOf(SalaryRecordType.DEDUCTION));
        assertThat(deductionsList.get(0).getAmount()).isEqualTo(EMPLOYEE_EXTRA_LEAVE_DAYS_DEDUCTED_MONEY);
    }
